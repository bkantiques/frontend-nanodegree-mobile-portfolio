...
var resizePizzas = function(size) {
  ...
  // Removed determineDx function- used simpler solution
...
function updatePositions() {
  ...
  // Calculate 5 possibilities for phase outside of loop
  var phases = [];
  var scrollTop = document.body.scrollTop;
  for(var j = 0; j < 5; j++) {
    phases[j] = Math.sin((scrollTop / 1250) + (j % 5));
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // Calculate number of moving pizzas that will fit on screen vertically
  var rows = Math.ceil(window.innerHeight / s);
  var numMovers = rows * cols;
  for (var i = 0; i < numMovers; i++) {
    ...
    // Positioning now done by transformX so left should be 0
    elem.style.left = "0px";
